...
Evaluated checkpoints preserve weights, optimizer, sampler RNG, torch RNG, step, seed, tokenizer, dataset identity and checkpoint-parent hash. The research trainer now supports `--resume <checkpoint.pt>` with a single matching seed in the supplied configuration. `steps` is the total target update count, not an additional-update count. Only future evaluation points, target steps and the per-invocation deadline may change; optimizer, data, model, tokenizer and generation settings are fixed. The original three-seed experiment remains historical and must not be extended using its inspected holdout.

Selection history is embedded in new checkpoints and checked against the hashed checkpoint and validation files. Keep ancestor run directories when moving or resuming a run; paths currently remain absolute. Original Phase 2A checkpoints reconstruct history from their original sibling artifacts. The saved boundary precedes validation, so that boundary is deterministically rescored after resume. An emergency checkpoint already containing that boundary's evaluation does not duplicate it. All continuation work goes into a new run; failed runs and the original random baseline remain intact.

Tests compare four uninterrupted updates with two plus two resumed updates and require exact model, optimizer, torch RNG, sampler RNG and validation agreement. Separate tests cover legacy history reconstruction, deadline interruption and rejection of changed inputs/history. This demonstrates local deterministic CPU continuation; cross-device and cross-version bitwise reproducibility remain untested.
